...
- envs/, folder containing the environment needed for the Snakefile to run. Need to make one specifically for MAC2 as MACS2 uses python 2.7 following the information found [here](https://groups.google.com/forum/#!searchin/snakemake/macs%7Csort:relevance/snakemake/60txGSq81zE/NzCUTdJ_AQAJ).
...
1. Create a virtual environment named "dmc1" from the `environment.yaml` file with the following command: `conda env create --name dmc1 --file ~/envs/global_env.yaml`
...
## Configuration file
The `~/config/config_tomato_DMC1.yaml` file specifies the sample list, the genomic reference fasta file to use, the directories to use, etc. This file is then used to build parameters in the main `Snakefile`.
...
# Main outputs
The main output are for now sorted bam files. This is a working version, use `git branch` to implement new rules.
